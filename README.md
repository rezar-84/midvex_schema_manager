...
For day-to-day setup and operating instructions, see [USAGE.md](USAGE.md).
...
- No Python Excel package is required for configuration transfer; the module reads and writes simple `.xlsx` workbooks directly.
...
Templates can mark **Load optional fields by default**. This loads useful optional fields automatically, while **Add Optional Fields** still adds missing optional rows later without duplicates. **Sync Template Fields** refreshes template rows and adds missing template fields without deleting custom rows.
...
- Global records: `website_id + target_type + lang_code + schema_type`
- Website page records: `website_id + target_type + website_page_id + lang_code + schema_type`
- Custom URL records: `website_id + target_type + normalized target_url + lang_code + schema_type`

Custom URL targets are normalized before save. For example, `https://example.com/about/`,
`/about/`, and `/about` are treated as the same render target. The same schema type can
be used on multiple different URLs.
...
Auto-fill reads translated Odoo website SEO fields for the schema record language first. For example, a `tr` schema record reads Turkish page metadata when the website has a matching installed Odoo language such as `tr_TR`. If translated metadata is missing, Odoo falls back to the source/base value.
...
Open **Website → Configuration → Structured Data → Import / Export Configs** to transfer reusable configuration between databases.

Supported formats:

- **Excel Workbook (`.xlsx`)**: one workbook with separate sheets.
- **CSV Files (`.zip`)**: a ZIP archive containing related CSV files.

Supported configuration:

- Global Website Schema settings
- Schema templates
- Model mappings and mapping lines

The importer expects the structure generated by the exporter. It updates existing records by stable keys where possible:

- Settings by resolved website
- Templates by `name + schema_type`
- Model mappings by mapping name
- Mapping lines are replaced for the imported mapping

Page Schema records are intentionally not part of this configuration transfer. They are page/content instances rather than reusable configuration. Use normal Odoo import for page-specific schema records, or use Model Mappings for dynamic schema generation across many records.

For safety, import files are limited to 5 MB, archives to 64 members, and individual archive members to 2 MB. Unknown CSV files inside an import ZIP are ignored.
...
Use model mappings when one schema shape should render across many products, blog posts, events, slides, pages, or partners. The mapping is evaluated at request time and fills values from the current target record plus safe tokens such as `{{ current.canonical_url }}`, `{{ product.name }}`, or `{{ blog.author }}`.
...
- Schema records should describe content visible to users on the target page. Do not add FAQ, Review, Product, Article, or local-business markup for hidden, fake, outdated, or unrelated content.
...
- Configuration imports are manager-only and protected by size and archive-member limits.

See [AUDIT_ROADMAP.md](AUDIT_ROADMAP.md) for the current product/security assessment and implementation roadmap.
