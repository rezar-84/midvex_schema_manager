...
CSV_DATASET_NAMES = {'settings', 'templates', 'mappings', 'mapping_lines'}
...
class MidvexSchemaConfigTransferWizard(models.TransientModel):
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(payload), 'r') as archive:
                ...
                for info in infos:
                    ...
                    if name not in CSV_DATASET_NAMES:
                        continue
                    content = self._archive_read_limited(archive, info).decode('utf-8-sig')
    ...
    def _read_xlsx(self, payload):
        ...
        with archive:
            ...
            workbook = self._safe_xml_from_archive(archive, 'xl/workbook.xml')
            rels = self._safe_xml_from_archive(archive, 'xl/_rels/workbook.xml.rels')
            rel_map = {
            ...
            for sheet in workbook.findall('.//main:sheet', _NS):
                ...
                rows = self._read_sheet_rows(
                    self._archive_read_limited(archive, self._safe_archive_info(archive, path)),
                    shared_strings,
                )
                if not rows:
                    datasets[name] = []
    ...
    def _validate_import_payload(self, payload):
        if len(payload or b'') > MAX_IMPORT_BYTES:
            raise ValidationError('Import file is too large. Maximum size is 5 MB.')

    def _validate_archive_members(self, infos):
        if len(infos) > MAX_ARCHIVE_MEMBERS:
            raise ValidationError('Import archive contains too many files.')
        for info in infos:
            if info.file_size > MAX_ARCHIVE_MEMBER_BYTES:
                raise ValidationError('Import archive contains a file larger than 2 MB.')

    def _safe_archive_info(self, archive, path):
        normalized = (path or '').replace('\\', '/').lstrip('/')
        if not normalized or normalized.startswith('../') or '/../' in normalized:
            raise ValidationError('Import workbook contains an invalid internal path.')
        try:
            return archive.getinfo(normalized)
        except KeyError as exc:
            raise ValidationError('Import workbook is missing "%s".' % normalized) from exc

    def _archive_read_limited(self, archive, info):
        if info.file_size > MAX_ARCHIVE_MEMBER_BYTES:
            raise ValidationError('Import archive contains a file larger than 2 MB.')
        return archive.read(info.filename)

    def _safe_xml_from_archive(self, archive, path):
        try:
            return ET.fromstring(
                self._archive_read_limited(archive, self._safe_archive_info(archive, path))
            )
        except ET.ParseError as exc:
            raise ValidationError('Import workbook contains malformed XML in "%s".' % path) from exc

    def _read_shared_strings(self, archive):
        if 'xl/sharedStrings.xml' not in archive.namelist():
            return []
        root = self._safe_xml_from_archive(archive, 'xl/sharedStrings.xml')
        values = []
    ...
    def _read_sheet_rows(self, payload, shared_strings):
        try:
            ...
            raise ValidationError('Import workbook contains a malformed worksheet.') from exc
        rows = []
